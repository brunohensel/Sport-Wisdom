...
class SearchAdapter(private val onClick: (TeamDto) -> Unit) : ListAdapter<TeamDto, SearchViewHolder>(SearchAdapter) {
  ...
  class SearchViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun bind(data: TeamDto, onClick: (TeamDto) -> Unit) {
      with(itemView){

        imgAddToFavorite.setOnClickListener { onClick(data) }
