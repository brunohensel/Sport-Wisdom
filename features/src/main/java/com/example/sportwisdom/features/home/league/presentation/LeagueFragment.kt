...
import com.example.sportwisdom.domain.reducer.home.HomeIntents.FetchLeagues
...
class LeagueFragment : Fragment(R.layout.fragment_league) {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    lifecycleScope.launchWhenCreated { viewModel.process(listOf(FetchLeagues(args.sportDto.strSport)).asFlow()) }
      ...
      viewModel
        ...
        .map { leagueState ->
          when (leagueState.syncState) {
            ...
            is LeagueSyncState.Message -> displayErrorMessage((leagueState.syncState as LeagueSyncState.Message).msg)
          }
