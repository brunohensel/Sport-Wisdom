...
import com.example.sportwisdom.domain.reducer.home.HomeIntents.FetchEvents
...
class EventsFragment : Fragment(R.layout.fragment_events) {
  ...
  private var eventDateDto = com.example.sportwisdom.domain.model.EventDateDto()
  private val eventsAdapter by lazy { EventsAdapter(::setSchedule) }

  override fun onCreate(savedInstanceState: Bundle?) {
    super.onCreate(savedInstanceState)

    lifecycleScope.launchWhenCreated { viewModel.process(flowOf(FetchEvents(args.leagueId))) }
      ...
      viewModel
        ...
        .map { eventState ->
          when (eventState.syncState) {
            ...
            is EventSyncState.Message -> displayErrorMessage(eventState.syncState)
          }
  ...
  private fun displayErrorMessage(msg: EventSyncState) {
    val m = msg as EventSyncState.Message
    progressBarEvents.isVisible = false
    Snackbar.make(requireView(), m.msg ?: "Unknown error", Snackbar.LENGTH_LONG).show()
  }
  ...
  private fun setSchedule(eventDto: EventDto) {
    ...
    eventDto.setEventDateAndTime()?.let {
      ...
      lifecycleScope.launch {
        ...
        viewModel.process(flowOf(com.example.sportwisdom.domain.reducer.home.HomeIntents.InsertEvent(eventDto)))
      }
